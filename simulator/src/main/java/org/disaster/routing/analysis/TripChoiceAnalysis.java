...
final class TripChoiceAnalysis {
    ...
    public TripChoiceAnalysis(Table persons, Table trips, List<String> modeOrder) {
        ...
        boolean hasWeight = persons.containsColumn(TripStatsDisaster.ATTR_REF_WEIGHT);

        for (Row trip : trips) {

            String person = trip.getText("person");
            int n = trip.getInt("trip_number") - 1;
            double weight = hasWeight ? trip.getDouble(TripStatsDisaster.ATTR_REF_WEIGHT) : 1;

            String predMode = trip.getString("main_mode");
            String[] split = trip.getString(TripStatsDisaster.ATTR_REF_MODES).split("-");

            if (n < split.length) {
                String trueMode = split[n];
                data.add(new Entry(trip.getObject(TripStatsDisaster.ATTR_REF_ID),
